...
const scriptSrc = "script-src 'self' " + hashes.join(' ') + ' https://embed.tawk.to https://*.tawk.to;';
const updated = headers.replace(/script-src [^;]*;/, scriptSrc);
if (updated === headers && !headers.includes(scriptSrc)) {
  throw new Error('Could not locate the script-src directive in _headers');
}
headers = updated;
